...
def train_tsp(args):
    ...
    GAOptim = GA.GAOptimizer(generations=20, mutation_rate=0.1, population_size=10, elite_pct=0.1)
    ...
    for generation in range(GAOptim.generations):
        ...
        for individual in GAOptim.population :
            ...
            #wandb.log({'Validation reward': out})
    ...
    #wandb.log({'Average tour length': fitness['best_model_index']})
...
if __name__ == '__main__':
    ...
    #wandb.init(project="PFA ERL", mode='online')          
    #wandb.run.name = "test PFA"
    ...
    exp_dir = (
            Path("./submitit-res")
            / f"{d.strftime('%Y-%m-%d')}gen10_rand_eval"
    )
